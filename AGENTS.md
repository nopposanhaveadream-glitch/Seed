...
セッション開始時に必ず参照:
- docs/CURRENT_STATE.md（現在地の短いコンパス）
- docs/CONTEXT_PROTOCOL.md（圧縮後・新セッション時の復帰手順）
- docs/ROADMAP_v3_draft.md（現在の再整理ロードマップ）
- docs/ROADMAP.md（v2履歴。フェーズC終了までの経緯）
...
- docs/session_log_2026-05-05_aor_implementation_completion.md（AOR実装完了と未投入判断の履歴）
...
## 現在のフェーズ
Phase 1 のフェーズC は 2026-05-01 13:55:45 に graceful shutdown で終了。

2026-05-06 時点では、AOR（観察層、Action Outcome Recorder）は commit `820dbc6` で実装済みで、Seed0 は AOR 統合済みの `core/agent.py` で実機再起動済み。`~/.seed0/memory/action_outcomes.db` に観察データを蓄積している。

現在の位置付けは、環境2（遊び場）へ直進するフェーズDではなく、環境1.5「観察と予測の身体化」。中心問いは「遊ばせる前に、遊びから学べる身体がいる」。
...
優先度3（フェーズD 進行の正式判断）は、v3ロードマップ再整理により「環境軸と認識層軸を分けて再判断する」扱いに変更。AORは **環境1 の延長として** 実装完了済み（2026-05-05、詳細は docs/session_log_2026-05-05_aor_implementation_completion.md）。
...
- core/ — 代謝エージェント（Phase 1 完了、AOR統合済み）
...
## 運用体制
- オーナーは目的・違和感・方向性を自然文で伝える
- Codex Desktop が設計整理、実装タスク化、実装、検証を主に担う
- Claude Code は必要時の精密な事実確認・レビュー・限定タスクに使う
- オーナーが厳密な実装仕様を書くことを前提にしない
- ただし、報酬関数・代謝パラメータ・行動プリミティブ・LLM接続・環境拡張のような第一原則に触る変更は、必ずオーナーと意図照合する
