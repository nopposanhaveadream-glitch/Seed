...
11. **プロセス重複事故と対応（2026-04-18〜19）**: macOSアップデート再起動時に旧プロセスが停止せず、新旧2プロセスが約18時間並行動作。`save_q_table()`の全置換操作が競合し、Q状態数が60→59に減少（1状態消失）。4/19にPIDファイル（`~/.seed0/agent.pid`）による二重起動防止を実装し、再発リスクをゼロに。ただしこの18時間のQ値は2プロセスが交互に上書きしていたため、数値の一貫性に疑義がある。5月上旬の振り返りで、この汚染期間が行動パターンに影響しているか検討予定。
