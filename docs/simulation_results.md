...
## v5: 構造バグ修正 + 行動コスト再設定（2026-04-09）

### 経緯

v4-Aで実機稼働後、30分時点でVE=16.4を観測して「安定」と判断したが、**VE推移を3点以上で確認しなかった**。実際にはVE=100→0への一方的な下降カーブの途中の1点に過ぎず、38分後にVE=0に到達。収支分析により、**行動コストが全支出の75%**を占めていることが判明。

### 発見された問題

#### 1. 行動コストに実測根拠がなかった

8行動中6行動が実装上no-op（何もしないでreturnするだけ）にもかかわらず、VEコスト0.1〜2.0が設定されていた。実測結果:

| 行動 | 旧VEコスト | 実CPU消費 | 実態 |
|------|------:|------:|------|
| sense_body | 0.1 | 0.1μs | no-op |
| sense_deep | 0.5 | 0.0μs | no-op |
| diagnose | 0.5 | 0.0μs | no-op |
| sleep | 0.5 | 0.0μs | no-op |
| write_memory | 0.3 | 0.0μs | no-op |
| adjust_priority | 0.5 | 0.0μs | no-op |
| purge_memory | 1.0 | 700μs | syncコマンド |
| clean_temp | 2.0 | 14μs | ファイル削除 |

比較: `read_all_sensors()`（毎ステップ無料で実行）は8,492μs。

#### 2. 毎ステップ自動記憶（第一原則違反）

`conscious.py`で行動に関係なく毎ステップSTMに経験を保存していた。write_memory行動の存在意義がなく、STMが毎ステップ+1で成長して維持コストが雪だるま式に増加。N≈420で条件1（VE維持）が破綻。

#### 3. sleep行動が失敗してもVEを消費

疲労<30では眠れないが、VEコスト0.5は先に消費される。32回の無駄な試行で16VE（全行動コストの22%）を浪費。

#### 4. 強制睡眠に到達不能

「疲労≥95 かつ VE≤5」のAND条件。VEは38分で枯渇するが疲労は95まで11.5時間かかり、同時に満たされない。VE=0で11時間動けないデッドロック。

### v5の修正内容

| 修正 | 内容 |
|------|------|
| 行動コスト | no-op 6行動→VEコスト0。purge_memory→0.004、clean_temp→0.001（BMC基準で実測CPU消費から導出） |
| 自動記憶 | 廃止。write_memory選択時のみSTMに保存 |
| sleep可否 | 疲労≥30でのみ選択肢に表示 |
| 強制睡眠 | VE条件を撤廃。疲労≥95のみで発動 |

### 収支分析

修正後の1ステップ（5秒）あたりVE変化:

```
restステップ: +dt×(r − b·f − N·m/60) = +0.035 VE（記憶0件時）
非restステップ: −dt×(b + N·m/60) = −0.050 VE（記憶0件時、行動コスト≈0）
均衡rest比率: 58.8%（活動41.2%）
```

旧v4-Aでの均衡rest比率は93%（活動7%）だった。
...
実機はシミュレーションよりVE=0比率が大幅に低い。Q学習が「VEが低くなりすぎる前に眠る」パターンを学習した可能性がある。2日目（4/10）のVE=0比率は**0.00%**。

### 開発プロセスの教訓

この修正過程で以下の教訓を得て、CLAUDE.mdの開発プロセスに追加した:

1. **紙の上で収支計算を先にやる** — コードを書く前に全収入・全支出を洗い出す
2. **フルシステムでシミュレーションする** — サブシステムだけのテストで安心しない
3. **検証は「傾き」を見る** — 3点以上の時系列で傾向を確認。1点のスナップショットで安定を判断しない
4. **第一原則との照合** — 実装がハードコードされた動作になっていないか確認
5. **失敗モード分析** — 本番投入前に「何が壊れうるか」を明示的にリストアップ

PRINCIPLES.mdに原則10「均衡から逆算して設計する」を追加。
- 「望ましい均衡」ではなく「構造が破綻しない条件」からパラメータを逆算する
- 均衡点がどこに落ち着くかはSeed0の創発に委ねる
...
| パラメータ | 値 | 根拠 |
|-----------|-----|------|
| **base_rate** | **0.01 VE/秒** | v2テスト1 |
| **rest_ve_recovery_rate** | **0.012 VE/秒** | v4-A確定 |
| **rest_bmc_fraction** | **0.5** | v4-A確定 |
| **行動VEコスト** | **no-op: 0、purge: 0.004、clean: 0.001** | v5: 実測CPU消費から導出 |
| **base_fatigue_rate** | **0.0023 /秒** | v2テスト2 |
| **sleep_fatigue_recovery_rate** | **0.010 /秒** | v2テスト2 |
| **sleep_ve_recovery_rate** | **0.02 VE/秒** | 設計書通り |
| **sleep_bmc_fraction** | **0.3** | 設計書通り |
| **強制睡眠条件** | **疲労≥95のみ** | v5: VE条件撤廃（デッドロック解消） |
| **sleep行動可否** | **疲労≥30** | v5: 失敗時VE浪費の防止 |
| **記憶保存** | **write_memory時のみ** | v5: 第一原則準拠 |
| **alpha (RunningBaseline)** | **0.001** | v2テスト3 |
| **記憶コスト率** | **0.001 VE/件/分** | v2テスト4 |
| **pressure_trim閾値** | **10 / 3** | v2テスト4 |
| **epsilon (初期/最小)** | **0.3 / 0.05** | v2テスト5 |
...
5. ~~v5: 構造バグ3件修正 + 行動コスト再設定~~ → 実機24h稼働でVE=0比率0.88%
6. ~~フェーズA: v5の96時間安定性確認~~ → 合格（VE=0比率安定）
7. ~~フェーズB: センサー拡張（4種追加）~~ → 完了（4/12）
8. ~~purge_memory構造バグ修正~~ → STMを実際に削減するよう変更（4/14）。記憶最大325→176件に改善、VE=0比率7.91%→0.00%
9. フェーズC: Phase 1長期観察中（4/19〜5月上旬）
10. フェーズC進行中（2026-04-15〜）: purge修正後48時間でVE=100に初到達（入眠VE 0→7→30→47→84→83 と改善）。4/17時点で稼働約206時間、Q学習60状態/273エントリ、ε=0.050（下限到達）
11. **プロセス重複事故と対応（2026-04-18〜19）**: macOSアップデート再起動時に旧プロセスが停止せず、新旧2プロセスが約18時間並行動作。`save_q_table()`の全置換操作が競合し、Q状態数が60→59に減少（1状態消失）。4/19にPIDファイル（`~/.seed0/agent.pid`）による二重起動防止を実装。ただしこの18時間のQ値は2プロセスが交互に上書きしていたため、数値の一貫性に疑義がある。

**2026-05-02 追記**: 5月上旬振り返り後の事実確認で、並走時間 約18時間24分、重複ステップ 12,890件、step後退 8,846件と判明。さらに、4/18 14:50 の2プロセス並走時には PIDファイルによる防御機構自体がまだ実装されていなかったことが判明（実装は 2026-04-19 11:26:53 のコミット `f099080`）。当時の事象は「防御の失敗」ではなく「防御機構の未実装期間に発生」した事象である（原因はログから判定不能）。「再発リスクをゼロに」という当初評価は、4/18 の事象（未実装期間に発生）を含めて評価すると正確ではない。詳細は `docs/known_issues_2026-05-01.md` を主出典として参照。

**2026-05-02 追記（実装強化）**: 当初の PID 値生存確認方式（`os.kill(pid, 0)`）には TOCTOU・PID再利用・異常終了時残留・非atomic書込の4脆弱性が確認されたため、`fcntl.flock(LOCK_EX | LOCK_NB)` ベースに置き換え（commit `3adb880`）、4脆弱性を解消した。詳細は `docs/known_issues_2026-05-01.md` 7章および `~/.seed0/reports/flock_failure_mode_analysis_2026-05-02.md`。
