...
**2026-04-18 14:50 に2プロセス並走の事象が確認されている**（約18時間24分の並走、重複ステップ12,890件）。当時はこの防御機構がまだ実装されておらず（実装は 2026-04-19 11:26:53 のコミット `f099080`）、機構の不具合ではなく未実装期間に発生した事象である。原因はログから判定不能。実装の脆弱性修正はフェーズD進行と独立に着手予定。詳細は `docs/known_issues_2026-05-01.md`。
