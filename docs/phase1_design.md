...
この値は「何もしなければ約2.8時間でVEが0になる」ように設定。
VE回復なしでは生存できない時間スケールを作り出す。
...
def sleep_fatigue_recovery(dt_seconds: float) -> float:
    """睡眠中の疲労減少。約2時間で疲労が100→0に回復する。"""
    recovery_rate = 0.014  # ≈ 100 / 7200秒
...
|------|------------|
| メインループ1回 | 5秒 |
| VEが100→0（何もしない場合） | 約2.8時間 |
| VEが0→100（睡眠中） | 約1.6時間 |
| 疲労が0→50（通常活動） | 約6時間 |
| 疲労が0→95（高活動） | 約3〜4時間 |
| 疲労が100→0（睡眠中） | 約2時間 |
| 1日の覚醒/睡眠サイクル | 覚醒5〜8時間 / 睡眠2〜3時間（状況次第） |
| Q学習のepsilon 0.3→0.15 | 約10時間 |
| Running baselineの適応 | 約1.5時間 |
| comfort zoneの完全自律化 | 約1日（初期値がほぼ上書きされる） |

> **注意:** 上記はすべて概算値。シミュレーション検証で「壊れない組み合わせ」を見つけてから確定する。

---

## 10. シミュレーション検証計画

Phase 1の実装前に、以下をシミュレーションで検証する。

### 検証項目

1. **VE枯渇テスト**: base_rateを変えて、VEが0になるまでの時間を確認。短すぎると何もできない、長すぎると有限性が感じられない。
2. **睡眠サイクルテスト**: 疲労蓄積率と回復率の組み合わせで、自然な覚醒/睡眠サイクルが生まれるか。
3. **comfort zone適応テスト**: Phase 0データを入力として、RunningBaselineが妥当な範囲に収束するか。
4. **Q学習収束テスト**: 報酬設計で学習が進むか。「何もしない」ばかりに落ち着かないか。
5. **記憶コスト均衡テスト**: 記憶維持コストとVE回復のバランス。記憶を全て失う前に睡眠で回復できるか。
6. **緊急モードテスト**: comfort zone逸脱時に緊急モードのリソース解放が適切に機能するか。

### 検証方法

- Phase 0の実データ（58,164件）をセンサー入力としてリプレイ
- パラメータの組み合わせをグリッドサーチ
- 「壊れる条件」（VEが0で復帰不能、疲労が100で回復不能、記憶が全消失）を特定し、その条件を避けるパラメータ範囲を確定

### 検証の原則（原則9）

シミュレーションでは「壊れない構造」を見つける。「良い振る舞い」は検証しない。振る舞いはすべて実環境での創発に委ねる。
