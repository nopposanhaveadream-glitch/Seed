...
- rest行動でVEを回復できる（食事に相当。0.012 VE/秒 + BMC半減）
- sleep中にVEを高速回復できる（0.017 VE/秒）

> **シミュレーション検証結果（v1→v2→v4-A）:** 当初はVE回復を睡眠のみとしていたが、覚醒時間の78%がVE=0になる構造的欠陥が判明。rest行動に「食事」としてのVE回復効果を追加。v2（回復0.005）では実機でVE=0が83%に張り付く問題が残り、v4-Aで回復率0.012＋BMC50%軽減に修正して正味+0.007/s（ストレス倍率1.0時）を確保。rest選択率80%、VE=0が0.6%に改善。
...
この値は、rest行動（VE回復 0.012 VE/秒 + BMC50%軽減）と組み合わせて安定的に動作する。
rest中の正味VE回復は +0.007 VE/秒（ストレス倍率1.0時）。VE 0→15 に約36分。
非rest行動中は 0.01 × ストレス倍率 + 行動コスト の消費。
...
ACTIONS = {
    ...
    "rest": Action(
        ...
        ve_cost=0.0,
        cooldown_sec=0,
        # 休憩＝食事。VEを回復する（0.012 VE/秒 + BMC50%軽減）。
        # rest中は他の行動ができない（食事中は動けない）。
        # rest中のBMCは通常の50%に軽減される。
        # 正味回復: +0.007 VE/秒（ストレス倍率1.0時）
        # 疲労の蓄積は通常通り（起きているため）。
        # v4-Aで確定（v1→v2→v4-Aの3段階で調整）。
    ),
...
Seed0が「エネルギー枯渇状態にどれだけ張り付いているか」を測る指標。低いほど代謝が健全。

**定義**: 覚醒中のステップのうち、VE < 0.1 だったステップの比率。

```
VE=0比率 = (覚醒中かつVE<0.1だったステップ数) / (覚醒中の総ステップ数)
```

**除外する対象**:
- 睡眠中のステップ（睡眠中はVEが低いのが正常なため）
- sleep行動への遷移ステップ（過渡状態のため）

**計算単位**:
- 日次レポート: その日の24時間分で計算
- 全期間平均: 稼働開始からの累積で計算

**閾値の根拠**: VE < 0.1 を枯渇と定義するのは、VE = 0.0 は浮動小数点演算で厳密にヒットしにくいため。0.1未満 ≈ ほぼエネルギーなし、という運用定義。実装は `core/daily_report.py` の `ve < 0.1` で判定している。
...
agent.pyはPIDファイル（`~/.seed0/agent.pid`）で二重起動を防止する。2026-04-17にmacOSアップデート後のプロセス重複が実際に発生し、ログの交互書き込みとセンサーのグローバル変数競合を引き起こしたため、対策として追加した。

- **起動時**: PIDファイルが存在し、そのPIDのプロセスが生存中なら起動を拒否する。プロセスが死んでいれば古いPIDファイルを削除して続行する。
- **終了時**: `_shutdown()` およびPythonの`atexit`でPIDファイルを削除する。異常終了時も可能な限り削除される。
- **代謝構造への影響**: なし。プロセス管理のインフラであり、VE・疲労・行動選択・報酬計算には一切関与しない。

**2026-04-18 14:50 に2プロセス並走の事象が確認されている**（約18時間24分の並走、重複ステップ12,890件）。当時はこの防御機構がまだ実装されておらず（実装は 2026-04-19 11:26:53 のコミット `f099080`）、機構の不具合ではなく未実装期間に発生した事象である。原因はログから判定不能。詳細は `docs/known_issues_2026-05-01.md`。

**2026-05-02 追記**: PID 値生存確認方式（`os.kill(pid, 0)`）には TOCTOU・PID再利用・異常終了時残留・非atomic書込の4脆弱性が確認された（`~/.seed0/reports/pid_bypass_investigation_2026-05-02.md`）。これを `fcntl.flock(LOCK_EX | LOCK_NB)` ベースに置き換え（commit `3adb880`）、4脆弱性を解消した。失敗モード確認は `~/.seed0/reports/flock_failure_mode_analysis_2026-05-02.md`。
...
|------|------------|------------------|
| メインループ1回 | 5秒 | — |
| rest中のVE正味回復 | +0.007 VE/秒 | v4-Aで確定 |
| VE 0→15（rest連続） | 約36分 | v4-Aで確定 |
| 非rest行動中のVE消費 | 0.01 × ストレス倍率 + 行動コスト VE/秒 | v2で確定 |
| VEが0→100（睡眠中） | 約1.6時間 | v1/v2で確認 |
| 疲労が0→95（通常活動） | 約10時間 | v2で確定 |
| 疲労が95→0（睡眠中） | 約2.4時間 | v2で確定（recovery_rate=0.010） |
| 1日の覚醒/睡眠サイクル | 覚醒5-6h × 5回 / 睡眠0.8-1.3h × 5回 | v4-Aで確定 |
...
| テスト | 結果 | 備考 |
|--------|------|------|
| VE枯渇 | **v1欠陥→v2改善→実機で再発→v4-Aで確定** | rest回復0.012 + BMC50%軽減 |
| 睡眠サイクル | **正常** | v4-A: 覚醒5-6h/睡眠0.8-1.3h × 5回/日 |
...
- v2: rest回復0.005追加 → シミュレーション上VE=0が0%に（見かけ上の解消）
- **実機Phase 1初回起動で再発**: 覚醒中VE=0が83%。rest正味-0.005/sでは回復不可能だった
- v3: rest回復0.008 + BMC70%軽減 → 正味+0.001/s（遅すぎる。中央値VE=0.1）
- **v4-A: rest回復0.012 + BMC50%軽減 → 正味+0.007/s（VE=0が0.6%、rest80%に改善）**

### 壊れる条件

- **VE:** rest正味回復+0.007/sで安定。ストレス倍率1.5超で回復が追いつかなくなる
...
## PIDファイル実装の既知の制約（2026-04-20確認 / 2026-05-02 解消）

`ensure_single_instance()` は `os.kill(pid, 0)` によるPIDの生存確認のみで、そのプロセスが本当にSeed0かどうか（プロセス同一性）は検証していない。

理論上、以下の条件が重なるとPID再利用による誤判定が発生しうる:
1. Seed0が異常終了してPIDファイルが残る
2. macOSが短時間のうちに同じPIDを別プロセスに割り当てる
3. そのプロセスがSeed0の起動タイミングで生きている

ただし失敗モードは「起動拒否」方向（= 起動しないだけ）であり、Q値汚染のような内部を静かに壊す事故にはならないため、実害は軽微。発生確率もmacOSのPID循環範囲（数万〜10万）を考えると極めて低い。

改善案として `ps` コマンドでコマンド名を照合する方法があるが、現時点の事故率に対してコード追加のコストが見合わないため保留。今後の改善候補として記録のみ残す。

**2026-05-02 追記**: 二重起動防止を `fcntl.flock(LOCK_EX | LOCK_NB)` ベースに置き換えた（commit `3adb880`）。生存中のプロセスがロックを保持しているため、PID 値の照合自体が不要となった。本セクションで記録していた PID再利用問題は実質的に発生しない構造になり、`ps` コマンドによる同一性照合の改善案も不要となった。詳細は `docs/known_issues_2026-05-01.md` 7章。

### 4/19実装後の動作実績

4/19〜20のテスト・運用で7回の起動/停止イベントについては期待通り動作（stale PID削除、二重起動拒否、正常終了時のPIDファイル削除）。

**ただし、2026-05-02 の事実確認で 4/18 14:50 の2プロセス並走事象が判明した**（当時 PID 防御機構は未実装、実装は 2026-04-19 11:26:53 のコミット `f099080`）。本セクションの「動作実績」（4/19〜20）は機構実装後の期間のみを対象としており、未実装期間（4/18）に発生した並走事象は含まれていない。詳細は `docs/known_issues_2026-05-01.md`。
