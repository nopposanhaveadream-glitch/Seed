...
**ただし、2026-04-18 14:50 にこの防御をすり抜けた事象が確認されている**（約18時間24分の二重プロセス並走、重複ステップ12,890件）。原因はログから判定不能。実装の脆弱性修正はフェーズD進行と独立に着手予定。詳細は `docs/known_issues_2026-05-01.md`。
...
**ただし、2026-05-02 の事実確認で 4/18 14:50 のすり抜け事象が判明した**（PID 防御が機能しないまま2プロセス並走）。本セクションの「動作実績」は 4/18 のすり抜け事象を含めて評価し直す必要がある。詳細は `docs/known_issues_2026-05-01.md`。
